...
# 创建视频写入器
for i in range(100):
    data = h5py.File(f"/home/lvhuaihai/EgoMimic/datasets/video/data_groceries_video/episode_{i}.hdf5", "r")
    ...
    video_frames = np.asarray(data['img_front'])
    import os
    os.makedirs("/home/lvhuaihai/human_videos", exist_ok=True)
    # mp4_name = f"/home/lvhuaihai/robot_videos/episode_{i}_left.mp4"
    mp4_name = f"/home/lvhuaihai/human_videos/episode_{i}_high.mp4"
    ...
    for binary_image in video_frames:
        ...
        bgr_frame = cv2.cvtColor(binary_image, cv2.COLOR_RGB2BGR)
