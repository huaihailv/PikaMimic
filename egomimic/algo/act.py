...
class ACTModel(nn.Module):
    ...
    def __init__(
        ...
        # self.aux_encoder_action_proj = nn.Linear(self.action_dim, hidden_dim)
        
        # self.aux_action_head = nn.Linear(hidden_dim, self.action_dim)
    ...
    def forward(self, qpos, actions, image, is_pad=None, env_state=None):
        return self._forward(
                ...
                is_pad=is_pad,
                # aux_encoder_action_proj=self.aux_encoder_action_proj,
                # aux_action_head=self.aux_action_head
        )
...
class ACT(BC_VAE):
    ...
    def load_state_dict(self, state_dict):
        """
        加载外部传入的状态字典到类的内部属性中
        :param state_dict: 外部传入的状态字典，格式通常为字典类型
        """
        print(self.nets["policy"].__dict__)
        for key, value in state_dict.items():
            # 检查类中是否有对应的属性，若有则进行更新
            new_key = re.sub(r'^nets.policy.', '', key)
            if hasattr(self.nets["policy"], new_key):
                setattr(self.nets["policy"], new_key, value)
            # else:
                # print(f"Warning: Key {key} in state_dict not found in the class, skipping update.")
    ...
    def _robomimic_to_act_data(self, batch, cam_keys, proprio_keys):
        qpos = [batch["obs"][k] for k in proprio_keys] # ee_pose / joint_positions
        ...
        actions = batch[self.ac_key] if self.ac_key in batch else None # actions_xyz_act / actions_joints_act
    ...
    def log_info(self, info):
        ...
        # log["L1_Loss"] = info["losses"]["action_loss"].item()
...
class TestModel:
    def __init__(self, config_path):
        ...
        ckpt_path = "/home/lvhuaihai/EgoMimic/experiment_results/ACT_egomimic_test/None_DT_2024-12-22-00-14-34/models/last.ckpt"
        ...
        self.act_algo.load_state_dict(state_dict)
    ...
    def run_test(self):
        ...
        hdf5_file = h5py.File("/home/lvhuaihai/EgoMimic/datasets/ego.hdf5", "r")
        ...
        batches = {
            'obs': {},
            'actions_joints_act': None
        }
        
        demo_0_joint_positions = torch.from_numpy(hdf5_file['data/demo_0/obs/joint_positions'][:])
        demo_1_joint_positions = torch.from_numpy(hdf5_file['data/demo_1/obs/joint_positions'][:])
        batches['obs']['joint_positions'] = torch.cat([demo_0_joint_positions, demo_1_joint_positions], dim=0)

        # 处理front_img_1数据，从HDF5中读取并转换为torch.Tensor后拼接
        # 这里假设front_img_1的数据在HDF5中存储格式可以直接转换为torch.Tensor，若需要额外处理请根据实际调整
        demo_0_front_img_1 = torch.from_numpy(hdf5_file['data/demo_0/obs/front_img_1'][:])
        demo_1_front_img_1 = torch.from_numpy(hdf5_file['data/demo_1/obs/front_img_1'][:])
        batches['obs']['front_img_1'] = torch.cat([demo_0_front_img_1, demo_1_front_img_1], dim=0)

        # 处理actions_joints_act数据，从HDF5中读取并转换为torch.Tensor后拼接
        demo_0_actions_joints_act = torch.from_numpy(hdf5_file['data/demo_0/actions_joints_act'][:])
        demo_1_actions_joints_act = torch.from_numpy(hdf5_file['data/demo_1/actions_joints_act'][:])
        batches['actions_joints_act'] = torch.cat([demo_0_actions_joints_act, demo_1_actions_joints_act], dim=0)

        # Process the batch for training
        # batch = self.act_algo.process_batch_for_training(batches, 'actions_joints_act')

        print("Input Batch:", batches)

        # Move batch to device
        batch = self.to_device(batches, self.act_algo.device)
